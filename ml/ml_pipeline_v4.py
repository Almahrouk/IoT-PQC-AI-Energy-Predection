...
"""
=====================================================================
PQC Energy Prediction — Corrected ML Pipeline v4
=====================================================================
Fixes vs v3:
1. IoT feasibility table uses median instead of mean
2. SHAP display fixed (shows % only, not raw log-ns values)
3. Output directory updated to ml_v4
=====================================================================
"""
...
STRUCT = {
    ...
    "ML-DSA-44"           : (256, 0, 0, 2),
    "ML-DSA-65"           : (256, 0, 0, 3),
    "Falcon-512"          : (512, 1, 0, 1),
    "Falcon-1024"         : (1024, 1, 0, 5),
    "RSA-2048"            : (2048, 0, 0, 3),
...
# ─────────────────────────────────────────────────────────────────
# 6. SHAP (fixed: show % only, not raw log-ns values)
# ─────────────────────────────────────────────────────────────────
try:
    import shap
    print("\n" + "=" * 62)
    ...
    for f, v in shap_s.items():
        bar = "█" * int(v / total * 40)
        print(f"  {f:25s}  {v/total*100:6.1f}%  {bar}")
    pd.DataFrame({"feature": shap_s.index,
                  "mean_abs_shap": shap_s.values,
...
# ─────────────────────────────────────────────────────────────────
# 7. IoT FEASIBILITY (fixed: uses median, not mean)
# ─────────────────────────────────────────────────────────────────
print("\n" + "=" * 62)
...
print(f"  IoT blocker:   Falcon keygen           (~900ms-2600ms on ARM ❌)")
