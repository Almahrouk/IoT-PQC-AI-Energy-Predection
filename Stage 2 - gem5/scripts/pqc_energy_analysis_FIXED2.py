...
def page_group_kfold(df_enc, pdf):
    ...
    watermark(fig)
    pdf.savefig(fig, facecolor=BG); plt.close(fig)
    return dict(zip(labels, zip(r2_means, r2_stds)))
...
def main():
    ...
    with PdfPages(args.output) as pdf:
        ...
        gkf_results = page_group_kfold(df_enc, pdf); print('    ✓ Page 7 — Cross-Algorithm GroupKFold (Exp B)')
    ...
    for lbl, (r2_mean, r2_std) in gkf_results.items():
        clean_lbl = lbl.replace('\n', '_')
        rows_ml.append({
            'exp':            f'B_{clean_lbl}',
            'model':          'Random Forest (GroupKFold)',
